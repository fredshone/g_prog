...
def evolve(rank_function,
	...
	population = [trees.make_random_tree(functions, inputs, depth=4) for i in range(pop_size)]

	for i in range(maxgen):
		scores = rank_function(population)
		print(f"gen {i + 1}, best score: {scores[0][0]}")
		if scores[0][0] == 1:
			break
		...
		while len(new_population) < pop_size:
			if random() > pnew:
				...
				new_population.append(trees.make_random_tree(functions, inputs, depth=3))
